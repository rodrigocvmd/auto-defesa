...
import { 
  ...
  CheckCircle, AlertTriangle, PenTool, Download, Send, RotateCcw, FileCheck, User, Car, MapPin, Gauge, Info,
  Search, Lock, FileText, AlertCircle, Scale, Gavel, FileWarning, HelpCircle, ArrowDown, Upload, Copy, Coins
} from 'lucide-react';
...
const UploadDefense = () => {
     ...
     try {
        ...
        if (response.success) {
            ...
            if (extractedData.infractionCode && !extractedData.article) {
                ...
                } catch (ignore) { console.log("Failed to auto-fetch article from code"); }
            }
  ...
  const confirmEdit = () => { setShowEditWarning(false); setIsEditing(true); };
  const handleDownloadClick = () => { setShowDownloadConfirm(true); };
  ...
  const EditWarningModal = () => (
    <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
        <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl relative p-6">
            ...
            <p className="text-gray-600 mb-6">Use a ferramenta de edição manual apenas para corrigir <strong>erros pontuais</strong>. Para alterar o conteúdo, recomendamos utilizar a <strong>IA de Refinamento</strong>.</p>
  ...
  const DownloadConfirmModal = () => (
    <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
        <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl relative p-6">
            ...
            <p className="text-gray-600 mb-6">Esta será a versão final do seu documento. Após confirmar, você será redirecionado.</p>
            <div className="flex justify-end gap-3">
        ...
        <div className="max-w-5xl mx-auto py-8">
          <div className="sticky top-20 z-40 bg-white/90 backdrop-blur-md p-4 rounded-2xl shadow-lg border border-gray-200 mb-8 flex flex-col md:flex-row justify-between items-center gap-4 animate-in slide-in-from-top-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleDownloadClick} className="bg-green-600 text-white px-6 py-2 rounded-xl font-bold hover:bg-green-700 flex items-center gap-2 shadow-md"><Download size={18} /> Baixar PDF</button>
            </div>
          </div>
      ...
      <MainLayout>
        <div className="max-w-2xl mx-auto py-12 px-4">
          <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 animate-in slide-in-from-bottom-4 duration-500">
            <div className={`p-8 text-center ${analysisData.viability === 'Alta' ? 'bg-green-50' : 'bg-yellow-50'}`}>
              <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-white shadow-sm mb-4">
                {analysisData.viability === 'Alta' ? <CheckCircle size={40} className="text-green-600" /> : <AlertCircle size={40} className="text-yellow-600" />}
              </div>
              <h2 className="text-2xl font-black text-gray-900 mb-2">Viabilidade {analysisData.viability}</h2>
            ...
            <div className="p-8">
              ...
              <div className="space-y-4 mb-6">
                {analysisData.arguments.slice(0, 3).map((arg, idx) => (
                  <div key={idx} className="relative flex items-start gap-3 p-4 bg-gray-50 rounded-xl border border-gray-100 overflow-hidden max-h-[80px]">
              ...
              <div className="bg-blue-600 rounded-2xl p-6 text-white text-center shadow-lg shadow-blue-200">
                ...
                <p className="text-blue-100 text-sm mb-6">Nossa IA já estruturou toda a argumentação jurídica baseada nas teses acima. Baixe o documento final editável agora.</p>
                
                {!currentUser ? (
                    <div className="flex flex-col gap-3">
                            <p className="text-blue-100 text-sm mb-2 font-medium">Você precisa estar logado para gerar o documento final.</p>
                            <button 
                            onClick={() => {
              ...
              <button onClick={() => setStep('form')} className="w-full text-center text-gray-400 text-sm mt-6 hover:text-gray-600">Voltar e editar dados</button>
            </div>
          </div>
        </div>
      </MainLayout>
      ...
      <MainLayout>
        <div className="max-w-5xl mx-auto py-8">
          <header className="mb-8">
            ...
            <p className="text-gray-600">Nossa IA leu seu documento. Verifique e complete as informações abaixo.</p>
          ...
          <form onSubmit={handlePreAnalysis} className="space-y-8 bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
            {loading && (
              <div className="fixed inset-0 bg-white/80 z-[100] flex flex-col items-center justify-center p-4 text-center">
                ...
                <p className="text-gray-600 max-w-md">Identificando erros na multa e melhores argumentos de defesa.</p>
              </div>
            )}
            ...
            <section className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div><label className="label-form">Data Assinatura <span className="text-red-500">*</span></label><input name="signDate" value={formData.signDate} onChange={handleChange} onBlur={handleBlur} className={`input-form ${errors.signDate ? 'border-red-500' : ''}`} required />{errors.signDate && <p className="text-red-500 text-xs mt-1">{errors.signDate}</p>}</div>
              </div>
            </section>
            <div className="flex flex-col items-center gap-4 py-8">
              ...
              <p className="text-center text-gray-400 text-sm mt-3">Nenhum crédito será cobrado nesta etapa.</p>
            </div>
          </form>
        </div>
  ...
  return (
    <MainLayout>
      <div className="max-w-3xl mx-auto py-10">
        <div className="mb-8 text-center"><Link to="/" className="inline-flex items-center text-gray-500 hover:text-blue-600 mb-6 transition-colors"><ArrowLeft size={20} className="mr-1" /> Início</Link><h1 className="text-3xl font-bold text-gray-900">Análise de Documento</h1><p className="text-gray-500 mt-2">Envie a foto da multa. A IA irá preencher os dados para você.</p></div>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden relative">
          <div className="p-6 md:p-10">
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleUploadAndExtract(); }} className="space-y-8">
