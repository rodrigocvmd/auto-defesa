...
export const FormStep = ({
    ...
    return (
        <div className="max-w-5xl mx-auto py-8">
            {showLimitModal && <LimitExceededModal onClose={() => setShowLimitModal(false)} onProceed={(e) => handlePreAnalysis(e, true)} />}
            {showLoginPrompt && <LoginPromptModal onClose={() => setShowLoginPrompt(false)} formData={formData} source="upload" />}
            {showHardBlockModal && <HardBlockModal hardBlockInfo={hardBlockInfo} onClose={() => setShowHardBlockModal(false)} />}
            {showDivergenceModal && <DivergenceWarningModal onClose={() => setShowDivergenceModal(false)} analysisData={analysisData} />}
            {showCodeNotFoundModal && <CodeNotFoundModal onClose={() => setShowCodeNotFoundModal(false)} onManualEntry={() => setIsManualInfraction(true)} setFormData={setFormData} />}
            {showTestModal && <TestInfoModal onClose={() => setShowTestModal(false)} onConfirm={confirmTestMode} />}
            ...
            <form
                ...
                {loading && (
                    <div className="fixed inset-0 bg-white/80 z-[100] flex flex-col items-center justify-center p-4 text-center">
                        ...
                        <p className="text-gray-600 max-w-md font-bold">Analisando os dados para definir a viabilidade do recurso e possíveis teses a serem aplicadas...</p>
                    </div>
                )}
