...
import { ArrowLeft, CheckCircle, Download, PenTool, Loader2, AlertCircle, Info, Send, Lock } from 'lucide-react';
...
export const ResultStep = ({
    ...
    const { handleGeneratePDF, loading: pdfLoading } = usePdfGenerator(componentRef, formData, setShowDownloadSuccess);
    ...
    return (
        <div className="max-w-6xl mx-auto py-8">
            {showEditWarning && <EditWarningModal onClose={() => setShowEditWarning(false)} onConfirm={() => { setShowEditWarning(false); setIsEditing(true); }} />}
            {showDownloadConfirm && <DownloadConfirmModal onClose={() => setShowDownloadConfirm(false)} onConfirm={handleConfirmDownload} />}
            {showDownloadSuccess && <DownloadSuccessModal onClose={() => setShowDownloadSuccess(false)} />}
            {showPrintInstructionModal && <PrintInstructionModal onClose={() => setShowPrintInstructionModal(false)} onPrint={executePrint} />}
