...
export const ResultStep = ({
	...
	const [overrideCity, setOverrideCity] = useState(formData.signCity || "");
	const [overrideDate, setOverrideDate] = useState(formData.signDate || "");
	const [overrideName, setOverrideName] = useState(formData.name || "");
	...
	// Pre-process result to enforce signature block directly from state
	let processedResult = result || "";
	
	// Remove any existing signature block generated by AI
	const signatureRegex = /<p[^>]*>.*?Nestes termos, pede deferimento\.?.*?<\/p>[\s\S]*/i;
	if (signatureRegex.test(processedResult)) {
		processedResult = processedResult.replace(signatureRegex, "");
	} else {
		// Fallback se a IA não usou "Nestes termos", busca pela linha de assinatura
		const lineRegex = /<p[^>]*>.*?_{10,}.*?<\/p>[\s\S]*/i;
		if (lineRegex.test(processedResult)) {
			processedResult = processedResult.replace(lineRegex, "");
		}
	}

	const finalCity = overrideCity || "[CIDADE DE ASSINATURA]";
	const finalDate = overrideDate || "[DATA DE ASSINATURA]";
	const finalName = overrideName || "[NOME DO RECORRENTE]";

	processedResult += `
<p style="text-align: center; margin-top: 30px;">Nestes termos, pede deferimento.</p>
<p style="text-align: center; margin-bottom: 30px;">${finalCity}, ${finalDate}.</p>
<p style="text-align: center; margin-top: 40px; margin-bottom: 0;">___________________________________________________</p>
<p style="text-align: center; margin-top: 5px;">${finalName.toUpperCase()}</p>`;
	...
	const handleSaveQual = (newText, newCity, newDate, newName) => {
		setQualText(newText);
