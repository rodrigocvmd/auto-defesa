...
const ManualDefense = () => {
  ...
  const handleSearchCode = async () => {
    ...
    } catch (error) { setShowCodeNotFoundModal(true); } finally { setSearchingCode(false); }
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div className="flex gap-2 items-end">
                    ...
                    <div className="w-24">
                        ...
                        <input name="infractionSplit" value={formData.infractionSplit} onChange={handleChange} onBlur={handleBlur} className="input-form w-full text-center" placeholder="0" />
                    </div>
                    <div className="mt-auto pb-1">
                        <button type="button" onClick={handleSearchCode} className="bg-blue-100 text-blue-600 p-2.5 rounded-xl hover:bg-blue-200 transition-colors h-[42px] w-[42px] flex items-center justify-center" title="Buscar Código">{searchingCode ? <Loader2 className="animate-spin" size={20} /> : <Search size={20} />}</button>
                    </div>
                </div>
                {errors.infractionCode && <p className="text-red-500 text-xs mt-1">{errors.infractionCode}</p>}
                <div className="md:col-span-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label className="label-form">Amparo Legal</label>
                            <input name="article" value={formData.article} onChange={handleChange} readOnly={!isManualInfraction} className={`input-form ${!isManualInfraction ? 'bg-gray-50 cursor-not-allowed' : 'bg-white border-yellow-400'}`} placeholder={isManualInfraction ? "Digite o Artigo (ex: Art. 218, I, CTB)" : "Aguardando preenchimento do Código da Infração..."} />
                        </div>
                    </div>
                ...
                <div className="md:col-span-3">
                    ...
                    <input name="infractionDescription" value={formData.infractionDescription || ''} onChange={handleChange} readOnly={!isManualInfraction} className={`input-form ${!isManualInfraction ? 'bg-gray-50 text-gray-600 cursor-not-allowed' : 'bg-white border-yellow-400'}`} placeholder={isManualInfraction ? "Digite a descrição da infração" : "Aguardando preenchimento do Código da Infração..."} />
                </div>
